...
- 推荐 Node.js 24；兼容 Node.js 20.19+ 或 22.13+。Node.js 20 使用 `better-sqlite3` 兼容驱动，新版使用内置 SQLite。
- pnpm 11。
- Python 3.10+（仅重新采集彩绘资料或运行采集测试时需要，日常启动不需要）。
...
彩绘资料来源于[民航休闲小站的机队目录](http://www.xmyzl.com/?mod=jidui)，只提取明确的“当前机队彩绘／特别涂装”栏目。2026-09-10 扫描 25 页目录、197 个机队详情页，提取 35 家航司的 343 个注册号。不是所有详情页都有当前彩绘栏目，未收录不代表没有彩绘。

数据已存入 `data/liveries.sqlite`，包括注册号、涂装名称、航司、来源链接、原始证据行和采集时间。仓库保存可审查的 `server/data/xmyzl-liveries.json` 种子快照；首次读取空数据库时自动导入，SQLite 文件和抓取 HTML 缓存不提交 Git。

查询浦东／虹桥航班时按注册号匹配，兼容 `B2006`、`B-2006` 等写法。详情页显示彩绘来源和采集日期。没有注册号的航班不会靠航司或机型猜测涂装；资料库不提供航班计划，也不能证明飞机当天一定到访或仍使用该涂装。

后续手动更新（建议低频运行，不必随每次航班查询抓取）：
...
仅在采集成功后执行导入。默认 `pnpm liveries:collect` 复用 HTML 缓存以便断点继续，`--cached` 完全离线解析。存在采集失败或无法解析的栏目时，不替换上一份有效种子快照；报告位于 `.runtime/xmyzl-last-report.json`。导入使用事务和去重，完整栏目内消失的旧记录仅停用，不物理删除。

更新后普通刷新网页即可重算彩绘标记，仍复用航班快照，不必使用消耗额度的强制刷新。`GET /api/health` 的 `liveryDatabase` 返回已加载注册号数量和采集时间。

验证命令：`pnpm test:liveries`。字段、覆盖范围及源站数量差异见《彩绘机资料库设计说明.md》。
